...
- **[Forge Agent Gate](https://github.com/forgeorbital/forge-agent-gate)** [![GitHub Repo stars](https://img.shields.io/github/stars/forgeorbital/forge-agent-gate?logo=github&label=&style=social)](https://github.com/forgeorbital/forge-agent-gate) - Deterministic pre-action authorization gate for agent actions: checks each proposed action against a signed mandate (amount caps, allowlists, approval thresholds, allowed hours) before it executes, blocks or escalates out-of-policy actions with human-in-the-loop, and writes a cryptographically signed decision record. TypeScript, MIT, npm + MCP server.
